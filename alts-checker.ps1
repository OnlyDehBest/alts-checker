    ...
    try {
        ...
        foreach ($acc in $data.accounts.PSObject.Properties) {
            ...
            if ($info.username -and $info.uuid) {
                ...
                Write-Host ("UUID:tt{0}" -f $info.uuid) -ForegroundColor Gray
                Write-Host ("Type:tt{0}" -f $info.type) -ForegroundColor Gray
...
function Get-LauncherAccounts {
    ...
    try {
        ...
        foreach ($acc in $data.accounts.PSObject.Properties) {
            ...
            Write-Host ("UUID:tt{0}" -f $uuid) -ForegroundColor Gray
            Write-Host ("Type:tt{0}" -f $type) -ForegroundColor Gray
...
function Get-MultiMCAccounts {
    ...
    try {
        ...
        foreach ($acc in $data.accounts) {
            ...
            Write-Host ("UUID:tt{0}" -f $acc.uuid) -ForegroundColor Gray
            Write-Host ("Type:tt{0}n" -f $acc.type) -ForegroundColor Gray
        ...
        if (Test-Path $path) {
            ...
            try {
                ...
                foreach ($acc in $data.PSObject.Properties) {
                    ...
                    if ($acc.Value.uuid) {
                        Write-Host ("UUID:tt{0}" -f $acc.Value.uuid) -ForegroundColor Gray
                    }
                    Write-Host ""
    ...
    try {
        ...
        foreach ($entry in $data) {
            if ($entry.username -and $entry.uuid) {
                ...
                Write-Host ("UUID:tt{0}n" -f $entry.uuid) -ForegroundColor Gray
